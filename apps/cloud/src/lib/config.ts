...
export function getLocalAvatarConfig(): LocalAvatarConfig {
  const rawUrl =
    process.env.EVB_LOCAL_AVATAR_URL?.trim() ||
    process.env.EVB_LOCAL_AVATAR_ENGINE_URL?.trim();
  const allowDevFallback =
    process.env.NODE_ENV !== "production" ||
    (getRunMode() === "solo" &&
      getStoreBackend() === "memory" &&
      getQueueBackend() === "memory");
  const baseUrl = rawUrl || (allowDevFallback ? "http://localhost:5600" : "");
