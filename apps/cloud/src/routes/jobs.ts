...
import { selectAvatarProviderForJob } from "../providers/providerFactory";
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const selection = selectAvatarProviderForJob(process.env, mode, input);
    console.log(
      `[EVB] job accepted jobId=${job.id} mode=${mode} provider=${selection.provider.name} ` +
