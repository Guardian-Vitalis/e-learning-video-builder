...
import { CourseVideoProject, StubAvatarStyle, StubBackgroundStyle } from "@evb/shared";
...
export default function GeneratePanel({ project, onProjectUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const status = await getJob(job.jobId);
        if (cancelled) {
          return;
        }
  ...
  const handleRetryStatus = async () => {
    ...
    setErrorDetails(null);
    try {
      const nextStatus = await getJob(job.jobId);
