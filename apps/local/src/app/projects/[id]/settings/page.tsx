...
export default function SettingsPage({ params }: Props) {
  ...
  const cloudBaseUrl = process.env.NEXT_PUBLIC_CLOUD_API_BASE_URL?.trim() ?? "";
    ...
    <main className="section-stack">
      ...
      {cloudMissing && (
        <InlineErrorBlock
          ...
          details="Set NEXT_PUBLIC_CLOUD_API_BASE_URL in apps/local/.env.local to configure the preview generator (apps/cloud) and restart the local app."
        />
      )}
      {cloudMissing && (
        <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
          ...
          <pre className="mt-3 whitespace-pre-wrap rounded-md border border-slate-200 bg-white p-2 text-xs text-slate-600">
            {`NEXT_PUBLIC_CLOUD_API_BASE_URL=http://127.0.0.1:3002`}
          </pre>
        </div>
      )}
