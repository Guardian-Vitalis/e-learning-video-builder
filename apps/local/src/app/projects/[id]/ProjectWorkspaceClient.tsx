...
function buildSelectionPayload(
  ...
  return {
    selections: manifest.sections.map((section) => ({
      sectionId: section.sectionId,
      variationIndex: selections[section.sectionId] ?? 0
    }))
  };
}
...
export default function ProjectWorkspaceClient({ params, baseUrl }: Props) {
  ...
  const selectedClip =
    selectedClipId && playlist.length > 0
      ...
      : playlist[0];
  const previewMp4Url = selectedClip
    ? resolveArtifactUrl(cloudBaseUrl, selectedClip.mp4Path)
    : selectedPreviewLinks?.mp4Url;
  ...
  const loadPreviewCaption = async (clipId: string, url: string) => {
    if (!selectedPreviewJobId) {
  ...
  const loadPreviewCaptions = async (jobId: string) => {
    if (!cloudBaseUrl) {
      setCaptionError("Cloud API base URL is missing.");
      return;
    }
    const manifest = clipManifests[jobId];
    const clipVttPath = selectedClip?.vttPath;
    const fallbackUrl = previewLinks[jobId]?.vttUrl;
    const vttUrl = clipVttPath ? resolveArtifactUrl(cloudBaseUrl, clipVttPath) : fallbackUrl;
    if (!vttUrl) {
      return;
    }
    ...
    try {
      let res = await fetchWithRetry({
        ...
        isRetryableStatus,
        getUrlAfterRefresh: (job) => `${cloudBaseUrl}${job.artifacts?.vttPath ?? ""}`
      });
      if (!res.ok && manifest && selectedClipId) {
        await fetchJobForPreview(jobId);
        const refreshed = clipManifests[jobId];
        const updatedClip = refreshed
          ? buildPlaylist(refreshed, variationSelections[jobId] ?? buildVariationDefaults(refreshed)).find(
              (clip) => clip.id === selectedClipId
            )
          : null;
        if (updatedClip?.vttPath) {
          res = await fetch(resolveArtifactUrl(cloudBaseUrl, updatedClip.vttPath));
        }
      }
      if (!res.ok) {
  ...
  const handleExportZip = async (jobId: string) => {
    if (!cloudBaseUrl) {
      ...
      setExportError(previewGeneratorHints.title);
      setExportErrorDetails(previewGeneratorHints.details);
      return;
    }
    ...
    const fetchJobArtifacts = async () => {
      const job = await getJob(jobId);
      if (job.status !== "succeeded") {
        throw new CloudApiError(409, "Artifacts not ready");
      }
      if (!job.artifacts) {
        throw new CloudApiError(409, "Artifacts missing");
      }
      return job;
    };

    const fetchManifest = async (job: JobRecord) => {
      if (!job.artifacts?.manifestPath) {
        return null;
      }
      const res = await fetch(resolveArtifactUrl(cloudBaseUrl, job.artifacts.manifestPath), {
      ...
      return (await res.json()) as JobArtifactsManifest;
    };

    const downloadArtifacts = async (artifacts: {
      mp4Path: string;
      vttPath: string;
      srtPath: string;
    }) => {
      setExportStatus("Downloading MP4...");
      const mp4Res = await fetchWithRetry({
        url: `${cloudBaseUrl}${artifacts.mp4Path}`,
        refresh: async () => {
          const refreshed = await fetchJobArtifacts();
          return refreshed;
        },
        isRetryableStatus,
        getUrlAfterRefresh: (job) => `${cloudBaseUrl}${job.artifacts?.mp4Path ?? ""}`
      });
      if (!mp4Res.ok) {
        throw new Error(`mp4 ${mp4Res.status}`);
      }
      const contentLength = mp4Res.headers.get("content-length");
      if (contentLength && Number(contentLength) > 200 * 1024 * 1024) {
        setExportWarning("Large file. This may take a while.");
      }
      const mp4Buffer = await mp4Res.arrayBuffer();

      setExportStatus("Downloading captions...");
      const [vttRes, srtRes] = await Promise.all([
        fetchWithRetry({
          url: `${cloudBaseUrl}${artifacts.vttPath}`,
          refresh: async () => {
            const refreshed = await fetchJobArtifacts();
            return refreshed;
          },
        ...
        fetchWithRetry({
          url: `${cloudBaseUrl}${artifacts.srtPath}`,
          refresh: async () => {
            const refreshed = await fetchJobArtifacts();
            return refreshed;
          },
          isRetryableStatus,
          getUrlAfterRefresh: (job) => `${cloudBaseUrl}${job.artifacts?.srtPath ?? ""}`
        })
      ]);
      if (!vttRes.ok) {
        throw new Error(`vtt ${vttRes.status}`);
      }
      if (!srtRes.ok) {
        throw new Error(`srt ${srtRes.status}`);
      }
      const [vttText, srtText] = await Promise.all([vttRes.text(), srtRes.text()]);
      return { mp4Buffer, vttText, srtText };
    };

    const buildZip = async (payload: {
      ...
      const zip = new JSZip();
      zip.file("video.mp4", payload.mp4Buffer);
      zip.file("captions.vtt", payload.vttText);
      zip.file("captions.srt", payload.srtText);
      zip.file(
        "manifest.json",
        JSON.stringify(
          {
    ...
    const requestSelectionZip = async (
      manifest: JobArtifactsManifest,
      selections: Record<string, number>
    ) => {
      setExportStatus("Requesting selected export...");
      const res = await fetch(`${cloudBaseUrl}/v1/jobs/${jobId}/export-selection`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(buildSelectionPayload(manifest, selections))
      });
      if (!res.ok) {
    ...
    try {
      const job = await fetchJobArtifacts();
      const manifest = await fetchManifest(job);
        const blob = manifest
          ? await requestSelectionZip(
              manifest,
              variationSelections[jobId] ?? buildVariationDefaults(manifest)
            )
          : await buildZip(await downloadArtifacts(job.artifacts));
      ...
      const retryable =
        lower.includes("401") || lower.includes("403") || lower.includes("410");
      if (retryable) {
        try {
          const job = await fetchJobArtifacts();
          const manifest = await fetchManifest(job);
          const blob = manifest
            ? await buildManifestZip(
                manifest,
                variationSelections[jobId] ?? buildVariationDefaults(manifest)
              )
            : await buildZip(await downloadArtifacts(job.artifacts));
          ...
          setExportStatus(null);
          setExportError("Export failed after retry.");
          setExportErrorDetails(retryMessage);
          return;
        }
      }
      if (err instanceof CloudApiError && err.status === 404) {
        setExportError("Job not found in cloud. It may have expired.");
      } else if (err instanceof CloudApiError) {
        setExportError("Cloud unreachable. Start dev stack.");
      } else {
      ...
      <section className="card space-y-4">
        ...
        <details className="rounded-md border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
          <summary className="cursor-pointer font-medium text-slate-800">
            Run preview generator (apps/cloud)
          </summary>
