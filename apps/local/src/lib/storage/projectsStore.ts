...
function isValidProject(value: unknown): value is CourseVideoProject {
  if (!value || typeof value !== "object") {
    return false;
  }
  const project = value as CourseVideoProject;
  return (
...
function normalizeProject(project: CourseVideoProject): CourseVideoProject {
...
export function listProjects(): CourseVideoProject[] {
...
export function getProject(projectId: string): CourseVideoProject | null {
...
export function updateProject(
  patch: Partial<CourseVideoProject> & { id: string }
): CourseVideoProject {
  const store = readStore() ?? { version: 1, projects: [] };
