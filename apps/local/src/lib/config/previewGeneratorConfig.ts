...
const PREVIEW_GENERATOR_STEPS = [
  "Run: yarn workspace @evb/cloud dev",
  "Set: NEXT_PUBLIC_CLOUD_API_BASE_URL=http://localhost:4000 (apps/local/.env.local)",
  "Restart: yarn workspace @evb/local dev"
];

const PREVIEW_GENERATOR_MESSAGE =
  "MP4 previews are required to derive clip audio automatically (Local Avatar).";
...
export function getPreviewGeneratorUiHints() {
  ...
  return {
    ...
    restartHint: PREVIEW_GENERATOR_STEPS[2]
  };
}
